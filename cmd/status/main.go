...
func markstatus(s string) string {
	...
	// s for the string versions, sb for the byte slice version

	// Must test for any spaces here because many of the tests for XWIN and OWIN will have spaces
...
func main() {
	...
	xwins, owins, draws, illegal, play := 0, 0, 0, 0, 0
	...
	fmt.Printf("Total %d\nXWIN %d \nOWIN %d \nDRAW %d \nILLEGAL %d \nPLAY %d \nSUM %d\n", len(lookup), xwins, owins, draws, illegal, play, xwins+owins+illegal+draws+play)
